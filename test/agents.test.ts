...
describe("agents API / AI 担当者", () => {
  it("エージェント作成 → タスクの担当に設定 → 一覧で agent 絞り込みできる", async () => {
    ...
    const createdAgent = await agent.post(`/api/w/${wsPublicId}/agents`).send({ name: "リサーチ担当" });
    ...
    const task = await agent.post(tasks).send({ title: "調査タスク", assigneeAgentId: agentId });
    ...
    const filtered = await agent.get(`${tasks}?agent=${agentId}`);
    expect(filtered.status).toBe(200);
    expect(filtered.body.tasks).toHaveLength(1);
    expect(filtered.body.tasks[0].number).toBe(task.body.task.number);
  });

  it("担当者はユーザーとエージェントの両方指定で 400（相互排他）", async () => {
    const { agent, userId, wsPublicId } = await signupAgent();
    const createdAgent = await agent.post(`/api/w/${wsPublicId}/agents`).send({ name: "AI" });
    const agentId = createdAgent.body.agent.id;
  ...
  it("エージェント担当のタスクにユーザーを割り当てるとエージェントは外れる", async () => {
    ...
    const task = await agent.post(tasks).send({ title: "切替", assigneeAgentId: agentId });
    const number = task.body.task.number;

    const patched = await agent.patch(`${tasks}/${number}`).send({ assigneeId: userId });
  ...
  it("他ワークスペースのエージェントは担当に指定できない（400）", async () => {
    ...
    const bAgent = await b.agent.post(`/api/w/${b.wsPublicId}/agents`).send({ name: "B の AI" });
    const bAgentId = bAgent.body.agent.id;

    // A が自分の WS のタスクに B のエージェントIDを指定 → A の WS には存在しないので 400
    const res = await a.agent
      .post(`/api/w/${a.wsPublicId}/tasks`)
