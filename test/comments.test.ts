...
describe("comments API", () => {
  it("投稿 → 一覧 → 編集 → 削除の一連が通る", async () => {
    ...
    const task = await agent.post("/api/tasks").send({ title: "議題" });
    ...
    const list = await agent.get(`/api/tasks/${taskId}/comments`);
    ...
    const del = await agent.delete(`/api/tasks/${taskId}/comments/${commentId}`);
    expect(del.status).toBe(204);

    const after = await agent.get(`/api/tasks/${taskId}/comments`);
  ...
  it("空のコメントは 400（zod バリデーション）", async () => {
    const { agent } = await signupAgent();
    const task = await agent.post("/api/tasks").send({ title: "議題" });
    const res = await agent.post(`/api/tasks/${task.body.task.id}/comments`).send({ body: "" });
  ...
  it("存在しないタスクへのコメントは 404", async () => {
    const { agent } = await signupAgent();
    const res = await agent.post(`/api/tasks/999999/comments`).send({ body: "x" });
