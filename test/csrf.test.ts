...
// 個人ワークスペースの publicId も取得して返す（スコープ付きパス /api/w/:publicId/... 用）。
async function rawSignup(username: string) {
  ...
  const wsPublicId = ws.body.workspaces[0].publicId as string;
  return { agent, wsPublicId };
}

describe("CSRF 保護", () => {
  it("トークン無しの状態変更は 403（code=CSRF）", async () => {
    const { agent, wsPublicId } = await rawSignup("csrf-none");
    const res = await agent.post(`/api/w/${wsPublicId}/tasks`).send({ title: "CSRF なし" });
    expect(res.status).toBe(403);
    expect(res.body.error.code).toBe("CSRF");
  });

  it("トークン付きの状態変更は成功する", async () => {
    const { agent, wsPublicId } = await rawSignup("csrf-ok");
  ...
  it("GET（一覧取得）はトークン無しでも通る", async () => {
    const { agent, wsPublicId } = await rawSignup("csrf-get");
    const res = await agent.get(`/api/w/${wsPublicId}/tasks`);
  ...
  it("ログアウト→再ログイン後も /api/csrf は新しいトークンを発行し、変更系が通る", async () => {
    ...
    const { agent, wsPublicId } = await rawSignup(username);
