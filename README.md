...
**Delegaru** は Express（JSON API）+ React SPA + Prisma + PostgreSQL で作ったタスク管理アプリ。担当者に**人間メンバー**または**AI エージェント**を指定でき（＝人／AI へタスクを "delegate" する）、Claude 等からリモート MCP で操作できるのが特徴。3層構造（フロントエンド / バックエンド / データベース）で構成し、GitHub → Render で公開する。

## 機能

- アカウント登録 / ログイン / ログアウト（セッション認証）
- タスクの追加・編集・削除・完了切替
- タスクの項目: タイトル / 説明 / 状態（未着手・進行中・完了）/ 優先度（高・中・低）/ 期限 / タグ / 担当者
- 担当者は**人間メンバー**または**AI エージェント**を指定可（ワークスペースごとにエージェントを登録）
- タグ・AI エージェントの作成・編集・削除（ワークスペースごと・色付き）
- 状態・優先度・担当者（人間/AI）・タグでの絞り込み、期限・優先度・作成日での並び替え、キーワード検索
- 期限切れタスクの強調表示
- メイン画面: 左にダッシュボード、右にワークスペース一覧（並べ替え可能）
- データはワークスペースごとに分離（所属メンバーのみ閲覧／操作可能）
...
src/api/                 JSON API: http(共通) / schemas(zod) / auth / tasks / agents / tags / workspaces / dashboard / index
prisma/schema.prisma     データモデル
client/                  React SPA（Vite + TS）
  src/api/               fetch ラッパと各エンドポイント呼び出し
  src/queries/           TanStack Query フック
  src/components/        Header / Layout / WorkspaceLayout / WorkspaceNav / DashboardPanel / TaskItem / TaskForm / FilterBar
  src/pages/             Login / Signup / Home(ダッシュボード+WS一覧) / Tasks / TaskDetail / Calendar / Tags / Agents / Workspaces(メンバー)
```
...
- 公開ツール（`src/mcp/tools.ts`）: `list_tasks` / `get_task` / `list_agents` / `create_task` / `update_task` / `toggle_complete` / `delete_task` / `create_agent`

Claude Code から接続する場合は `.mcp.json`（本リポジトリの例）に URL だけ書けばよい（初回接続時にブラウザで OAuth 同意＝ログインが走る）:

```json
{ "mcpServers": { "delegaru": { "type": "http", "url": "https://my-app-zosl.onrender.com/mcp" } } }
```
